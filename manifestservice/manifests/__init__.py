...
def get_manifests():
    ...
    result, ok = _list_files_in_bucket(flask.current_app.config.get("MANIFEST_BUCKET_NAME"), folder_name)
    if not ok:
...
def get_manifest_file(file_name):
    ...
    json_to_return = {
        "body" : _get_file_contents(flask.current_app.config.get("MANIFEST_BUCKET_NAME"), folder_name, file_name)
    }
...
def put_manifest():
    ...
    if not is_valid:
        return flask.jsonify({"error" : "Manifest format is invalid. Please POST a list of key-value pairs, like [{'k' : v}, ...] Required keys are: " + " ".join(required_keys)}), 400
...
def _add_manifest_to_bucket(current_token, manifest_json):
    ...
    """
    session = boto3.Session(region_name="us-east-1", aws_access_key_id=app.config['AWS_ACCESS_KEY_ID'], aws_secret_access_key=app.config['AWS_SECRET_ACCESS_KEY'])
    ...
    filename = _generate_unique_manifest_filename(folder_name, flask.current_app.config.get("MANIFEST_BUCKET_NAME"), result)
    manifest_as_bytes = str.encode(str(flask.request.json))
    filepath_in_bucket = folder_name + "/" + filename

    try:
        obj = s3.Object(flask.current_app.config.get("MANIFEST_BUCKET_NAME"), filepath_in_bucket)
...
def _generate_unique_manifest_filename(folder_name, manifest_bucket_name, users_existing_manifest_files):
    ...
    filename = _generate_unique_filename_with_timestamp_and_increment(timestamp, existing_filenames)
    return filename

def _generate_unique_filename_with_timestamp_and_increment(timestamp, users_existing_manifest_files):
    """
...
def _get_file_contents(bucket_name, folder, filename):
    ...
    """
    client = boto3.client("s3", aws_access_key_id=app.config['AWS_ACCESS_KEY_ID'], aws_secret_access_key=app.config['AWS_SECRET_ACCESS_KEY'])
...
def _authenticate_user():
    ...
    try:
        auth_successful = _does_the_user_have_read_access_on_at_least_one_project(authutils_user.current_user.projects)
    except Exception as e:
        print(e)
